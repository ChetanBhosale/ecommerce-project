...
import { cartModel, cartProductModel, ICart, IcartProduct, IProduct, productModel } from "../model/product";
...
export const createProduct = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        const {name,description,price,less,color,size,category,state} = req.body as IProduct
...
export const uploadImage = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        ...
        productData?.images?.push({public_id : myCloud.public_id,url : myCloud.secure_url})
...
export const removeUploadedImage = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        const id = req.params.id;
        ...
        product.images = product.images?.filter(image => image.public_id !== public_id && image.url !== url) || [];
...
export const getProducts = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { page = 1, size = 10, sort = 'createdAt', order = 'desc', search = '', category='', state='', from = "0", to = "10000000" } = req.query;

    const fromPrice: number = parseFloat(from as string);
    const toPrice: number = parseFloat(to as string);

    
    const skip: number = (parseInt(page as string) - 1) * parseInt(size as string);
...
export const viewProductList = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        const { page = 1, size = 10, sort = 'createdAt', order = 'desc', search = '', category='', state='', from = "0", to = "1000000" } = req.query;
...
export const addToCart = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        ...
        let getCart = await cartModel.findOne({user : req.user._id}).populate('products')

        if(!getCart){
            return next(new ErrorHandler('invalid request',400))
        }
        console.log(getCart);
        let isProductNotInCart:IcartProduct = getCart.products.find((ele:any) => ele.product.toString() === id) as unknown as IcartProduct
        ...
        let newUpdatedCart:ICart = await cartModel.findById({_id : req.user.cart}).populate('products') as ICart
        let total = 0;
...
export const viewCart = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        ...
        let cart:ICart = await cartModel.findById({_id : req.user.cart})
                    .populate({
                        path : 'products',
                        populate : {
                            path : 'product',
...
export const removeFromCart = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const deleteProduct: IcartProduct | null = await cartProductModel.findByIdAndDelete(id);

        if (!deleteProduct) {
            return next(new ErrorHandler("product not found!", 400));
        }

        let cart: ICart | null = await cartModel.findById(req.user.cart)
            .populate({
                path: 'products',
                populate: {
                    path: 'product',
        ...
        cart.products = cart.products.filter((product: any) => product._id.toString() !== deleteProduct._id.toString());
