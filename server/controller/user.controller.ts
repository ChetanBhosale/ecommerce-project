...
import { accessTokenOption, refreshTokenOption, sendToken } from '../service/sentToken'
...
export const registerUser = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const activationToken = jwt.sign({ code,email,password,role }, AccessTokenSecret!, { expiresIn: '5m' });
...
interface IActivationData { email :string,password:string,role : string,code:string }
...
export const activeUser = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    ...
    try {
        ...
        if(!code || !activationToken){
            return next(new ErrorHandler('Invalid, Please try to send new code!', 400));
        }


       const newUser = jwt.verify(activationToken,AccessTokenSecret!) as IActivationData

        if(newUser.code !== code){
            return next(new ErrorHandler("Invalid Activation Code", 400));
        }

        const { email, password,role }:{email :string,password:string,role:string} = newUser;
...
export const loginUser = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        ...
        if(!email || !password){
            return next(new ErrorHandler('please provide correct email and password!',400))
        }
...
export const updateAccessToken = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        ...
        const decode = jwt.verify(refreshTokenData,RefreshTokenSecret) as JwtPayload
...
export const userDetails = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        res.json({
            data : req.user
        })
