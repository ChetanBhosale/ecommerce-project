...
export const isAuthenticated = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        ...
        const decode = await jwt.verify(access_token,AccessTokenSecret) as JwtPayload
        if(!decode){
...
export const isSeller = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
        if(req.user?.role === "SELLER"){
            ...
            return next(new ErrorHandler(`${req.user.role} is not allowed to get access to resources!`,400))
        }
    } catch (error:any) {
        return next(new ErrorHandler(error.message,400))
    }
...
export const isUser = CatchAsyncError(async(req:Request,res:Response,next:NextFunction) => {
    try {
